...
#  最強のクロスボウを持ち、レーザー発射中でない時、Shiftを
#  「押した瞬間」だけ反応する（立ち上がりエッジのみ）
# ============================================================
scoreboard players add @s uw.mode 0
scoreboard players add @s uw.sneakp 0

execute unless entity @s[tag=uw.laser] if score @s uw.sneakp matches 0 if predicate uw:sneaking if items entity @s weapon.mainhand *[minecraft:custom_data~{uw_crossbow:1b}] run function uw:crossbow/mode/apply
