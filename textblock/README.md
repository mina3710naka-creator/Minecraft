...
`pack.mcmeta` の `min_format` / `max_format` を **107 → 121** に上げただけです。
このデータパックは述語（predicate）やルートテーブルを使わず、マクロ関数（`$`）と
`data` / `fill` / `scoreboard` コマンドだけで組まれているため、26.3 の
データパック形式変更（ルートの `"condition"` キーが `"type"` に改名された件など）の
影響を受けませんでした。26.2 で使いたい場合は `min_format` / `max_format` を
`107` に戻してください。
