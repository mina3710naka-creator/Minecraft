...
| 5 | 見えない台車（コウモリ）にプレイヤーを **ride で乗せ**、その台車に本物の
...
* **本物の Motion で引き寄せる** — バニラのコマンドでは、実在する**プレイヤー**の
  ...
  `/ride` コマンドでプレイヤーをその台車に乗せます。台車は `Motion` を
  毎ティック直接書き換えることができ、乗っているプレイヤーはゲーム本来の
  物理演算で台車の動きにそのまま追従します。**`tp` で座標を上書きしている
  のではなく、本物の速度ベクトルで動かしている**ため、視点はプレイヤー
  自身が自由に操作したままで、移動だけが台車に委ねられます（乗っている間、
  WASD 等の自力移動は効きません）。
  * **台車には防具立てではなくコウモリ（`NoAI` で自力飛行だけ止めたモブ）を
    使っています。** 防具立てはノックバック等の外力を一切受け付けない
    バニラの仕様があり、`Motion` を書き換えても実際には動かないため
    （防具立てで試して「乗るけど何も起きない」場合はこれが原因です）。
    通常のモブは外力に反応する（＝ Motion の書き換えがそのまま効く）ので、
    確実に動かせるコウモリを採用しています。
  ...
  * 台車（コウモリ）は実体としての当たり判定を持つので、壁や床への衝突は
    tp で座標を無理やり書き換える方式と違い、**ゲーム本来の衝突判定で
  ...
  * **既知の制約（バニラの仕様上、コマンドでは解消できません）** —
    ...
    **座りポーズ**になる、という 2 点が必ず起こります。台車は無敵
    （`Invulnerable:1b`）で常に満タン表示のままなので実害はありませんが、
    見た目が気になる場合は tp ベースの移動方式に戻す必要があります
    （本物の Motion を諦める代わりに、この 2 点は発生しなくなります）。
