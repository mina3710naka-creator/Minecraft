...
* **プレイヤーを直接 tp せず、見えない台車に乗せて台車を tp する** —
  ...
  * **マーカーに到達すると、台車をすぐに片付け、爆発扱いのダメージで
    ...
    * ダメージそのものは事前に `minecraft:absorption` を大量に付与して
      吸収させているため、実質ノーダメージで済む（体力が少ないときに
      これが原因で死ぬことはない）。ウィンドチャージらしい見た目・音は
      `gust` パーティクルと `entity.wind_charge.wind_burst` サウンドを
      演出として別途鳴らして再現している。
