...
| 到達時の打ち上げの強さ | `pull/launch.mcfunction` | `summon minecraft:wind_charge` の行数（現在 2 発。増減で強さが変わる） |
...
* **プレイヤーを直接 tp せず、見えない台車に乗せて台車を tp する** —
  ...
  * **マーカーに到達すると、台車をすぐに片付け、ウィンドチャージの爆風で
    プレイヤーを打ち上げます**（`pull/arrive.mcfunction` /
    `pull/launch.mcfunction`）。当初は台車を tp で上向きに動かして
    プレイヤーを持ち上げる方式にしていたが、これはスクリプトで無理やり
    動かしているだけで動きが不自然だったため、**ウィンドチャージの爆風
    ノックバック**（プレイヤーにもきちんと働く、数少ない「本物の速度」を
    与える手段）で打ち上げる方式に変更した。プレイヤーを台車から降ろした
    直後、体の中心あたりにウィンドチャージを出現させ、下向きの速度
    （`Motion`）を与えて自分の体を貫通させる形で 2 発起爆させ、ブロック
    破壊なしの上向きの本物のノックバックを与えている（バニラの爆風
    なので、高さは厳密に 5 ブロックとは限らず、実際の物理演算に委ね
    られる）。3 秒経過による解除（到達できなかった場合）では打ち上げは
    発生しません。
    * ウィンドチャージは**静止したまま出しただけでは当たり判定が働かず、
      いつまでも起爆しないことがあります**（衝突判定が「移動した経路上に
      何かあったか」を見る仕組みのため）。そのため、出現直後に
      `data modify entity @s Motion set value [...]` で実際に下向きの
      速度を与え、確実に自分の体を通過させて接触・起爆させています。
