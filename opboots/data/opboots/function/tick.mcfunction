...
# --- 1) 装備状態の検知 ---
execute as @a[tag=!ob.worn] if items entity @s armor.feet *[minecraft:attribute_modifiers~{modifiers:[{id:"opboots:speed_boost"}]}] run function opboots:equip/on_equip
execute as @a[tag=ob.worn] unless items entity @s armor.feet *[minecraft:attribute_modifiers~{modifiers:[{id:"opboots:speed_boost"}]}] run function opboots:equip/on_unequip
