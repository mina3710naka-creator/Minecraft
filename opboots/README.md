...
真上に打ち上げます（フックショットの打ち上げ演出で実際に検証済みの値を2回重ねています）。
...
| ジャンプの高さ | `function/jump/leap.mcfunction` | `damage @s 6 minecraft:wind_charge at ~ ~-2 ~` の行数（＝重ねて撃つ回数）。増やすほど高く飛ぶ |
...
* **10マスジャンプの上昇方法（`tp` を使わない理由）** — `tp` でプレイヤーを直接動かす方式は、
  ...
  フックショットの引き寄せ到達時の打ち上げ（`pull/launch.mcfunction`）で実際に検証済みの
  `damage @s 6 minecraft:wind_charge at ~ ~-2 ~` を使っている。これは指定した座標を発生源とした
  本物の爆発ノックバックを、実際に何かへ着弾させることなく与えられるコマンドで、ブロックへの
  衝突が不要なため**地形に一切影響を与えず**、地面の有無にも左右されない（地上でも空中でも
  ...
  * 当初は独自に量（`10`）と距離（`~-1`）を調整した値や `minecraft:explosion` 種別を
    ...
    かえって弱くなってしまった。数値や種別を変えると結果が予測しにくいため、まずは
    フックショットで実際に動作確認されている**そのままの1回分の強さ**を使い、それを
    2回連続で撃つことで高さを積み増す方式にしている（1回だけだとフックショット側の
    README にある通りおおよそ5マス程度）。
  ...
  * 見た目・音（`particle minecraft:gust` / `playsound minecraft:entity.wind_charge.wind_burst`）は
    実際のエンティティを伴わない演出として別途鳴らしている。
  * **空中ジャンプで効果が弱く感じる場合** — 空中ジャンプは、すでに落下中で下向きの速度が
    ついている状態で発動することが多い。ノックバックはプレイヤーの現在の速度に上乗せされる
    ため、下向きの速度が大きいほど打ち消すのに使われてしまい、正味の上昇量が減って感じられる。
    プレイヤーの速度そのものをリセットするコマンドは存在しない（プレイヤーのNBTは
    `/data` で読み書きできない）ため、この対策としては上の表の通り `damage` の行を
    増やして１回あたりの合計の勢いを強くするしかない。
