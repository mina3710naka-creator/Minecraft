...
真上に打ち上げます。地形は一切壊れず、ダメージ自体もブーツの耐性・保険で実質無効になります。
...
| ジャンプの高さ | `function/jump/leap.mcfunction` | `damage @s 10 minecraft:explosion at ~ ~-1 ~` の量（`10`）と発生源までの距離（`~-1`）。量を増やす／距離を近づけるほど高く飛ぶ |
...
* **10マスジャンプの上昇方法（`tp` を使わない理由）** — `tp` でプレイヤーを直接動かす方式は、
  ...
  `damage @s 10 minecraft:explosion at ~ ~-1 ~` （フックショットの引き寄せ到達時の打ち上げ
  `pull/launch.mcfunction` と同じ発想のコマンド）を実行している。これは指定した座標を
  発生源とした本物の爆発ノックバックを、実際に何かへ着弾させることなく与えられるコマンドで、
  ブロックへ衝突させる必要がまったくないため**地形に一切影響を与えず**、地面の有無にも
  左右されない（地上でも空中でも同じように動作する）。バニラの物理演算にそのまま任せているので
  `tp` は一切使っていない（現在の値でおおよそ10マス相当）。
  ...
  * このブーツは常に耐性・被ダメージ無効化の保険がかかっているため、フックショット側で
    ...
    という細工（低HP時に実際の10ダメージで死なないようにするためのもの）は不要にしている。
  ...
  * バニラの物理演算に任せているため、正確に10.0マスになるとは限らない（フックショットの
    打ち上げ演出と同様、目安の値）。もっと高く／低くしたい場合は上の表の通り `damage` の量や
    発生源までの距離を調整する。
