...
さらに**空中では低速落下で自由に動き回れる**「最強のブーツ」を追加するデータパックです。
すべて `function`（コマンド）で実装しています。
10マスジャンプは `tp` を使わず、**ウィンドチャージを自分の足元に1個投げ込み、
...
| 不可壊 | ブーツの耐久値が減らない |
| Shift + ジャンプ | その場から真上に約10マスジャンプ（`tp` は使わず、足元に投げたウィンドチャージ1個の爆風のみ） |
| Shiftを3秒長押し + ジャンプ | 爆発の演出とともに周囲を吹き飛ばしつつ、同じ10マスジャンプを発動（本人はブーツの効果で無傷、ブロックは壊さない） |
| 空中にいる間（常時） | 低速落下がかかり続け、重力にほとんど縛られず自由に動き回れる（地面に着いた瞬間に切れる） |
...
| ジャンプの高さ | `function/jump/leap.mcfunction` | `summon minecraft:wind_charge ~ ~ ~ {...}` の行数（＝足元に投げるウィンドチャージの個数）。増やすほど爆風が強くなり高く飛ぶ。現在は1個でおおよそ10マス相当 |
| 多重発動を防ぐクールダウン | `function/jump/tick.mcfunction` | `ob.leapt matches 6..`（6tick=0.3秒。シフト押しっぱなしで連続ジャンプしないための待ち時間） |
| 空中での落下の遅さ | `function/air_control.mcfunction` | `effect give @s minecraft:slow_falling 1 0 true` のamplifier（`0`＝Lv.1） |
...
* **10マスジャンプの上昇方法（`tp` を使わない理由）** — `tp` でプレイヤーを直接動かす方式は、
  ...
  ウィンドチャージを1個、下向きの初速を持たせて出現させている。下向きに撃ち出されたウィンドチャージは
...
* **空中での自由な移動** — 空中にいる間だけ `minecraft:slow_falling`（低速落下）を毎tick
  かけ直し続けている（`function/air_control.mcfunction`）。低速落下がかかると落下速度が
  大幅に遅くなり（かつ落下ダメージも受けない）、その間もWASDでの移動入力はそのまま効くため、
  重力にほとんど縛られずゆっくり自由に動き回れるようになる。接地判定は
  `execute unless block ~ ~-1 ~ #minecraft:replaceable` で、足元のブロックが
  `#minecraft:replaceable`（空気や草など、乗ってもすり抜けるようなブロック）でなければ
  「接地」とみなす簡易判定（`ground_track.mcfunction`）。真のオンラウンド判定ではないため、
  階段やハーフブロックの端などではわずかにずれる場合がある。
  効果時間を短く（1秒）してあるので、着地した瞬間にすぐ低速落下が切れる。
...
└── data/
    ...
    └── opboots/
        ...
        └── function/
            ...
            ├── ground_track               … 接地判定
            ├── air_control                … 空中での自由な移動（低速落下）
            ├── give / uninstall
            └── jump/   … ジャンプ検知・通常/爆発ジャンプ・クールダウン管理
```
