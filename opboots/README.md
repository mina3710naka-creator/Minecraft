...
10マスジャンプ・空中ジャンプはどちらも `tp` を使わず、レビテーション（浮遊効果）の上昇力だけで
真上に打ち上げます。
...
| 内容 | ファイル | 該当箇所 |
| --- | --- | --- |
| 耐性・跳躍強化・速度・炎耐性の強さ | `function/effects.mcfunction` | `effect give` 各行のamplifier（例：`minecraft:speed 0.1 1 true` の `1` が速度Ⅱ） |
...
└── data/
    ...
    └── opboots/
        ...
        └── function/
            ...
            ├── give / uninstall
            └── jump/   … ジャンプ検知・通常/爆発/空中ジャンプ・レビテーション管理
```
