...
* **10マスジャンプの上昇方法（`tp` を使わない理由）** — `tp` でプレイヤーを直接動かす方式は、
  ...
  まず実行者（プレイヤー）の足元に静止したウィンドチャージを1個
  （`summon minecraft:wind_charge ~ ~-0.3 ~ {NoGravity:1b,...}`）置き、その直後に
  `data modify entity <対象> Motion set value [0.0,-1.0,0.0]` でそのウィンドチャージの
  移動速度を書き換えて真下へ叩き込んでいる。ウィンドチャージは自分自身がブロックや
  エンティティに衝突すると爆発する仕様なので、これによって狙った位置（プレイヤーの真下）で
  確実に爆風を起こしている。バニラの物理演算にそのまま任せているので `tp` は一切使っていない
  （現在の1個でおおよそ10マス相当）。
  * 最初は「矢を撃ち込んで起爆させる」方式を試したが、バニラの仕様上**矢は他の投射物
    （ウィンドチャージ自身も含む）には命中判定されない**ため起爆しなかった
    （矢が命中するのはボート・額縁・防具立てなど「拾える・持ち帰れる」扱いの実体のみ）。
    そのため、ウィンドチャージ自身に実際の速度を持たせてブロックへ衝突させる、
    より確実な方法に変更している。
  * `summon` した瞬間からいきなり下向きの初速を持たせても良いが、ここでは
    「まず足元に置く → 直後にMotionを書き換えて叩き込む」という2段階の処理にして、
    位置決めと起爆のタイミングを分けている。
  * バニラの物理演算に任せているため、正確に10.0マスになるとは限らない（フックショットの
    打ち上げ演出と同様、目安の値）。もっと高くしたい場合は上の表の通り `summon` の行を
    増やす（＝置くウィンドチャージの数を増やす）。
  * `function/jump/tick.mcfunction` は、着弾せずに残ってしまったウィンドチャージの後始末
    （`kill @e[type=wind_charge,tag=ob.wc,distance=..4]`）に加え、シフトを押しっぱなしに
